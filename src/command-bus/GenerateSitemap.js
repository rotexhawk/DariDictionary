...
export default class GenerateSitemap {
    ...
    execute() {
        return this.generateUrls(config, this.englishController, this.dariController)
            .then(urls => this.createIndex(urls));
    }

    generateUrls(config, englishController, dariController) {
        return Promise.all([dariController.getWords(-1), englishController.getWords(-1)]).then(values => {
            return values.map(words => {
