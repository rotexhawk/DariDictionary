...
export default class SubscriptionController {
    ...
    subscribeToDailyWord(req, res, next) {
        ...
        new Promise((resolve, reject) => {
            this.db.run(`INSERT INTO ${this.table}('firstName','lastName','email', 'activation_key', 'unsubscribe_key', 'active')
             VALUES(?,?,?,?,?,?)`, [...names, req.body.email, activationKey, unsubscribeKey, 0], err => {
                if (err) {
            ...
            this.mailer.sendActivationEmail(data)
                .then(msg => {
                    console.log(msg);
    ...
    unsubscribeFromDailyWord(unSubscribeKey) {
        return new Promise((resolve, reject) => {
            this.db.run(`update ${this.table} set active = 0 where unsubscribe_key = '${unSubscribeKey}'`, (err) => {
                if (err) {
                    reject(err);
                }
                resolve();
