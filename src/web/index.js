...
export default ({ config, readDB, subscribersDB }) => {
    ...
    api.get('/', (req, res, next) => {
        engWordController.getWordOfDay().then(data => res.render('index', data)).catch(next);
    });
    ...
    api.post('/', (req, res, next) => {
        if (/^[A-Za-z0-9]/.test(req.body.word)) {
            ...
            engWordController.getWordWithDetailsByString(req.body.word).then(data => res.render('index', data)).catch(err => {
                console.log('what the fuck');
                next(err)
            });
        } else {
            dariWordController.getWordWithDetailsByString(req.body.word).then(data => res.render('index', data)).catch(next);
        }
    });
    ...
    api.get('/word/:wordid(\\d+)', (req, res, next) => {
        engWordController.getWordWithDetailsById(req.params.wordid).then(data => res.render('index', data)).catch(next);
    });
    ...
    api.get('/word/:word([\\w+%20?\\-?]+)', (req, res, next) => {
        engWordController.getWordWithDetailsByString(req.params.word).then(data => res.render('index', data))
            .catch(error => {
                if (error) {
                    res.render(path.join('./pages/missing-word'), { word: req.params.word })
                }
            }).catch(next);
    });
    ...
    api.get('/word/:word([\\u0600-\\u06FF%20?\\-?]+)', (req, res, next) => {
        dariWordController.getWordWithDetailsByString(req.params.word).then(data => {
                data.wordType = 'Dari';
    ...
    api.get('/translate', (req, res, next) => {
        dariWordController.getWordForTranslation().then(data => res.render(path.join('./pages/translate'), data)).catch(next);
    });
    ...
    api.get('/confirm', (req, res, next) => {
        return confirmationController.confirm(req).then(result => {
    ...
    api.get('/unsubscribe/:unSubscribeKey', (req, res, next) => {
        subscriptionController.unsubscribeFromDailyWord(req.params.unSubscribeKey)
            .then(res.render(path.join('./pages/unsubscribe'))).catch(next);
    });
