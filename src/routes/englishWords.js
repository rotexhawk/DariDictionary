...
export default ({ config, readDB }) => {
    ...
    api.get('/:word([\\w+%20?]+)', (req, res, next) => wordController.getAutoSuggestions(req.params.word).then(res.json.bind(res)).catch(next));


    /** Same as word by id except it searches by string. */
    api.get('/details/:word([\\w+%20?]+)', (req, res, next) => wordController.getWordWithDetailsByString(req.params.word).then(res.json.bind(res)).catch(next));
