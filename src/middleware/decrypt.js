...
function parseBody(body) {
    ...
    body.forEach(resObj => { // Go through response and get single obj
        if (Array.isArray(resObj)) { // If response is an Array of Arrays call this method recursively to get the object
            parseBody(resObj);
        }
        for (let prop in resObj) { // Get object props.
            if (resObj[prop] !== null && resObj[prop].type === 'Buffer') { // if the object's prop is a buffer, decrypt it
