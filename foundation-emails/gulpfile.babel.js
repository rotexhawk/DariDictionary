...
gulp.task('build',
    gulp.series(clean, pages, sass, images, inline, transfer));
...
gulp.task('transfer',
    gulp.series('build', transfer));

/**
 *  If yarn build is triggered. Transfer the compiled files to our email templates folder and save them as nunj.
 **/
function transfer() {
    return gulp.src('dist/dictionary_layouts/*.html')
        .pipe($.if(PRODUCTION, $.dest('../src/views/email', { ext: '.nunj' })))
...
function watch() {
    ...
    gulp.watch('src/data/**/*').on('all', gulp.series(pages, inline, browser.reload));
}
